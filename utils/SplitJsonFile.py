...
def main(args):
    ...
    # small, medium, large -> ensemble 얘네는 어떻게 처리해야하는지?
    # 세 모델 값의 결과를 하나의 df로 만듦 -> 다른 모델의 결과와 ensemble 
    if args.split_size == 'small':
        ann_data = [ann for ann in annotations if ann['area'] < 32**2]
    elif args.split_size == 'medium':
        ann_data = [ann for ann in annotations if 32**2 <= ann['area'] < 96**2]
    elif args.split_size == 'large':
    ...
    with open(os.path.join(save_path, f'{args.ann_file.split(".")[0]}_{args.split_size}.json'), 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--split_size', '-s', type=str, default='small',
    )
    parser.add_argument(
